using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Storage;
using System;
using System.Collections.Generic;
using System.Linq;

namespace eShop.Loader
{
    public class eShopRepository : IShopRepository
    {
        private readonly eShopContext _context;

        public eShopRepository(eShopContext context)
        {
            this._context = context;
        }

        /// <summary>取得商品清單</summary>
        /// <returns></returns>
        public IEnumerable<ProductMain> GetProducts()
        {
            return this._context.ProductMains.ToList();
        }

        /// <summary>取得指定商品型號的商品庫存</summary>
        /// <param name="schema">指定商品型號</param>
        /// <returns>商品庫存</returns>
        public int GetProductValidStorage(string schema)
        {
            var _product = this._context.ProductMains
                                        .Where(x => x.Schema == schema)
                                        .Include(x => x.ProductStorage)
                                        .FirstOrDefault();

            if (_product == null)
                return 0;

            var _storage = _product.ProductStorage;

            if (_storage == null)
                return 0;

            if (_product.ProductStorage.Storage.HasValue == false)
                return 0;

            return _product.ProductStorage.Storage.Value;
        }


        public bool AddOrder(Guid memberGUID
                            , List<OrderDetail> items)
        {
            using (var _transaction
                    = this._context.Database.BeginTransaction())
            {
                try
                {
                    //購買商品是否還有足夠的庫存
                    foreach (var _item in items)
                    {
                        var _match = this._context.ProductStorages
                                                    .Where(x => x.ProductNo == _item.ProductNo)
                                                    .FirstOrDefault();

                        _match.Storage = Convert.ToInt16(_match.Storage - _item.Quantity);

                        if (_match.Storage < 0)
                        {
                            _transaction.Rollback();

                            return false;
                        }
                    }

                    string _orderSchema;

                    OrderMain _orderMain;
                    OrderDetail _orderDetail;

                    //建立商品訂單編號
                    _orderSchema = String.Format("{0:yyyyMMdd}{1,7}"
                                                , DateTime.Now
                                                , this.GetOrderSchemaSeq(_transaction));
                    _orderSchema = _orderSchema.Replace(" ", "0");

                    //新增購買商品訂單
                    _orderMain = new OrderMain();
                    _orderMain.No = this.GetOrderMainSeq(_transaction);
                    _orderMain.Schema = _orderSchema;
                    _orderMain.OrderDate = DateTime.Now;
                    _orderMain.MemberGUID = memberGUID;
                    _orderMain.IsDeleted = false;

                    this._context.OrderMains.Add(_orderMain);

                    foreach (var item in items)
                    {
                        _orderDetail = new OrderDetail();
                        _orderDetail.OrderNo = _orderMain.No;
                        _orderDetail.ProductNo = item.ProductNo;
                        _orderDetail.SellPrice = item.SellPrice;
                        _orderDetail.Quantity = item.Quantity;

                        this._context.OrderDetails.Add(_orderDetail);
                    }


                    this._context.SaveChanges();

                    _transaction.Commit();

                    return true;
                }
                catch (Exception ex)
                {
                    _transaction.Rollback();

                    return false;
                }
            }
        }



        //Orders.OrderMainSeq
        private int GetOrderMainSeq(IDbContextTransaction transaction)
        {
            return this.GetCurrentSequenceValue("Orders.OrderMainSeq", transaction);
        }

        //Orders.OrderSchemaSeq
        private int GetOrderSchemaSeq(IDbContextTransaction transaction)
        {
            return this.GetCurrentSequenceValue("Orders.OrderSchemaSeq", transaction);
        }

        /// <summary>取得指定 Sequence 名稱的數值</summary>
        private int GetCurrentSequenceValue(string sequenceName, IDbContextTransaction transaction = null)
        {
            using (var _cmd = this._context.Database.GetDbConnection().CreateCommand())
            {
                _cmd.CommandText = $"SELECT NEXT VALUE FOR {sequenceName}";

                if(transaction != null)
                    _cmd.Transaction = transaction.GetDbTransaction();

                this._context.Database.OpenConnection();
                var _result = _cmd.ExecuteScalar();
                this._context.Database.CloseConnection();

                return Convert.ToInt32(_result);
            }
        }

    }

}